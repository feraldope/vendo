...
//	}
	...
	public JFreeChart getChart (String chartTitle, OilUsageData oilUsageData) {
		...
		TimeSeries overallAverage = new TimeSeries (OverallAverage);
		...
		plot.setBackgroundPaint(Color.GRAY.brighter());

		if (true) { //debug
			System.out.println(XYPlot.DEFAULT_GRIDLINE_PAINT + " = XYPlot.DEFAULT_GRIDLINE_PAINT");
			System.out.println(Color.GRAY + " = Color.GRAY");
			System.out.println(Color.GRAY.brighter() + " = Color.GRAY.brighter()");
			System.out.println(Color.GRAY.darker() + " = Color.GRAY.darker()");
		}
		...
		//first disable default domain grid lines (which will be drawn by default in same place as labels, i.e., in the middle of the month)
...
//		plot.addRangeMarker (rangeMarker);
		...
		renderer.setBaseToolTipGenerator (new StandardXYToolTipGenerator () {
			...
			public String generateToolTip (XYDataset dataset, int series, int index) {
				...
				if (OverallAverage.equals(name)) {
					return name + ": " + _decimalFormat.format (number) + " gals/day";
				} else {
					RegularTimePeriod regularTimePeriod = timeSeries.getTimePeriod (index);
					Date date = regularTimePeriod.getStart ();

					return name + ": " + _dateFormatMonthDay.format (date) + ": " + _decimalFormat.format (number) + " gals";
				}
			}
		});
	...
	//private members
	private static final String OverallAverage = "Overall Average"; //needs special handling for tooltip

	private static final RegularTimePeriod _firstConcreteDayToPlot = new Day (1, 1, 2015); //convert logical dayOfYear into concrete day/year (but avoid a leap year)
